...
def normalize_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Flatten ocean nested dict column if present
    if 'ocean' in df.columns:
        ocean_df = df['ocean'].apply(pd.Series)
        for trait in ['openness', 'conscientiousness', 'extraversion', 'agreeableness', 'neuroticism']:
            if trait in ocean_df.columns:
                expanded = ocean_df[trait].apply(pd.Series)
                df[f'ocean_{trait}_score'] = expanded['score']
                df[f'ocean_{trait}_level'] = expanded['level']
        df = df.drop(columns=['ocean'])

    # Flatten parents_education if present
    if 'parents_education' in df.columns:
        parents_df = df['parents_education'].apply(pd.Series)
        df['parents_education_parent_1'] = parents_df['parent_1']
        df['parents_education_parent_2'] = parents_df['parent_2']
        df = df.drop(columns=['parents_education'])

    # Flatten hobbies list to string if present
    ...
    if 'edges' in df.columns:
        df['edges'] = [
            [tuple(e) for e in row_edges]
            if isinstance(row_edges, (list, tuple, np.ndarray))
            else row_edges
            for row_edges in df['edges']
        ]    

    # Original whitespace stripping
    str_cols = df.select_dtypes(include='object').columns
    # df[str_cols] = df[str_cols].apply(lambda c: c.str.strip())
    df[str_cols] = df[str_cols].apply(
        lambda c: c.map(lambda x: x.strip() if isinstance(x, str) else x)
    )
    cat_cols = df.select_dtypes(include='category').columns
    df[cat_cols] = df[cat_cols].astype(str).apply(lambda c: c.str.strip())
    df.attrs['mhds_normalized'] = True
